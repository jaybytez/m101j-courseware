...
public class MongoDBSparkFreemarkerStyle {
	public static void main(String[] args) {
		...
		configuration.setClassForTemplateLoading(
				MongoDBSparkFreemarkerStyle.class, "/freemarker");
		...
		Spark.get("/", (request, response) -> {
			...
			try {
				...
				// Not necessary yet to understand this. It's just to prove that
				// you
				// are able to run a command on a mongod server
				List<Document> results = collection.aggregate(
						asList(new Document("$group", new Document("_id",
								"$value").append("count", new Document("$sum",
								1))), new Document("$match", new Document(
								"count", new Document("$lte", 2))),
